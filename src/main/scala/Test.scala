...
object Test {
  def main(args: Array[String]): Unit = {
    ...
    val rp = Megawatts(2); val d = Meters(80);
    val cd = (0 to 200).map(i => Megawatts(i) * 0.1 / SquareKilometers(1)).toList
    def n(cd: Irradiance): Double = Math.sqrt(rp / ((cd) * (d * d)))
    def lambda(cd: Irradiance): Double = Math.PI / (4 * n(cd) * n(cd))
    def output(cd: Irradiance) = {
      cf * area * cd * GustavsonWakeEffect.wakeEffect(cd * area / rp, lambda(cd))
    }
    PlotHelper.plotXY(List((cd.map(_.toWattsPerSquareMeter), cd.map(output(_).toGigawatts), "")), xLabel = "Installed Capacity Density [MW/km2]",
      yLabel = "Mean Power Produced [GW]")

    println(world.grids.map(_.area.toSquareKilometers).sum / world.grids.size)
    world.writeGrid("verticalTransport")
    //   val offshore1000m = world.offshoreGrids.filter(_.waterDepth.toMeters <= 1000);
...
*/
  ...
  def plotGustavson {
    ...
    PlotHelper.plotXY(
      List((n, n.map(m => GustavsonWakeEffect.wakeEffect(5, lambda(m))), "5x5"),
        (n, n.map(m => GustavsonWakeEffect.wakeEffect(50, lambda(m))), "10x10"),
        (n, n.map(m => GustavsonWakeEffect.wakeEffect(200, lambda(m))), "50x50"),
        (n, n.map(m => GustavsonWakeEffect.wakeEffect(500, lambda(m), inf = true)), "Infinite")),
  ...
  def testOptimaleCD {
    ...
    val p = cfs.map(cf => {
      cd.map(c => {
        ...
        nT * Megawatts(2) * cf * Hours(365 * 24) * GustavsonWakeEffect.wakeEffect(nT, Math.PI / (4 * n * n))
      })
    })
