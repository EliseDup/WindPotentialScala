...
trait SolarTechnology {
  ...
  def lifeTimeEfficiency =  
    if(degradationRate==0) efficiency * performanceRatio
    else efficiency * performanceRatio * ((1.0 - math.pow(1.0-degradationRate, ee.lifeTime))/degradationRate)/ee.lifeTime
  def panelArea(ratedPower: Power): Area = ratedPower / (WattsPerSquareMeter(1000) * efficiency)
  def potential(solar: Irradiance, ratedPower: Power) : Power = panelArea(ratedPower) * solar * lifeTimeEfficiency
