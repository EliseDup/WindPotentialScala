...
object Components {
  def apply(file: String, startRow: Int, endRow: Int): Components = {
    val sheet = Helper.xlsSheet(file, 0)
    new Components {
      val components = (startRow to endRow).map(r => {
        val row = sheet.getRow(r)
        val mass = Helper.toDouble(row, 1)
        val intensity = Helper.toDouble(row, 3)
        (Kilograms(mass), Material(Helper.toString(row, 0), Megajoules(1.0 / intensity), Kilograms(1)))
      }).toList
    }
  }
}
 ...
 */
class DefaultWindTurbineComponents(val totWeight: Mass) extends Components {

  val blades = (totWeight * 0.027, Material("Glass fibre, epoxy,PVC", Megajoules(61.8), Kilograms(1)))
  val hub = (totWeight * 0.035, Material("Steel", Megajoules(36.8), Kilograms(1)))
  val transmission = (totWeight * 0.052, Material("Steel", Megajoules(36.8), Kilograms(1)))
  val generator = (totWeight * 0.026, Material("Copper", Megajoules(86.2), Kilograms(1)))
  val nacelle = (totWeight * 0.003, Material("Glass fibre", Megajoules(61.8), Kilograms(1)))
  val tower = (totWeight * 0.233, Material("Steel", Megajoules(36.8), Kilograms(1)))
  val foundations = (totWeight * 0.603, Material("Concrete", Megajoules(3.2), Kilograms(1)))
  val electrical = (totWeight * 0.021, Material("Copper", Megajoules(86.2), Kilograms(1)))
 ...
 */
class WindTurbineComponents(
    val foundation: Components,
    val tower: Components,
    val nacelle: Components,
    val rotor: Components) extends Components {
  val components = (foundation.components ++ tower.components ++ nacelle.components ++ rotor.components)
}
 ...
 *
 */
class WindTurbineComponent(comp: List[(Double, String)]) extends Components {
  val materials = Materials()
  val components = comp.map(c => (Tonnes(c._1), materials(c._2).get))
}
object WindTurbineComponent {
  def apply(list: (Double, String)*) = new WindTurbineComponent(list.toList)
}

class WindTurbine850kWComponents extends WindTurbineComponents(
...
class WindTurbine2MWComponents extends WindTurbineComponents(
...
class WindTurbine3MWComponents extends WindTurbineComponents(
