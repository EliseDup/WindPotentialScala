...
object LoadHourCalculation {
  def main(args: Array[String]): Unit = {
    ...
    val hoogrijkEstimation = v.map(i => Math.max(0, i * 565 - 1745)).toList
    val list = k.map(i => {
      println(i)
      (v, v.map(v => new Weibull(v, i).capacityFactor(turbine2MW) * 365 * 24).toList, "K factor: " + i)
    })
    PlotHelper.plotXY(list.toList :+ (v, euEstimation, "2 MW EU Report Estimation") :+ (v, hoogrijkEstimation, "Hoogrijg PHD Thesis"), xLabel = "Mean Wind Speed", yLabel = "Load Hours", legend = true)
