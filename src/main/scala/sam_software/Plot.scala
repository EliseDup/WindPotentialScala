...
object Plot {
  ...
  def main(args: Array[String]): Unit = {
    val dni = (500 to 3000).map(_.toDouble).toList
    plotXY(List( (dni, dni.map(d => CSPParabolic.efficiency(WattsPerSquareMeter(d/8.76))), "Trough no storage"),
        (dni, dni.map(d => CSPParabolicStorage12h.efficiency(WattsPerSquareMeter(d/8.76))), "Trough 12h"),
        (dni, dni.map(d => CSPTowerStorage12h.efficiency(WattsPerSquareMeter(d/8.76))), "Tower 12h")), legend = true)
  ...
  def eroi(dni: Double, sm: Double, efficiency: Double, technology: SolarTechnology): Double = {
    ...
    30 * annualOutput / (technology.ee.embodiedEnergyArea(ratedPower, annualOutput, area))
  }
