...
object OptimalCapacityDensity {
  def main(args: Array[String]): Unit = {
    ...
    val cells = loadCells(eroi_min, "../WindPotentialPy/res_grid0_75")
    plotOptimalCD(eroi_min,cells)
 
    val onshore = cells.filter(_.onshore)
    val offshore = cells.filter(i => !i.onshore)
    printResults(cells, List(2.0, 7.0, 12.0), topDown = true)
    printResults(onshore, List(2.0, 7.0, 12.0), topDown = true)
    printResults(offshore, List(2.0, 7.0, 12.0), topDown = true)

    println("Loading Time - " + (System.currentTimeMillis() - t) / 1000 + "seconds")
    // plotOptimalCD(eroi_min, cells)
    // plotEROIVSProd(eroi_min, cells, true)
    // val list = listValueVSCumulated(cells.map(i => (i.eroi(false, 0.0), i.annualProductionTopDown(false, 0.0).to(Exajoules))))
    // val listS = listValueVSCumulated(cells.map(i => (i.eroi(true, 0.0), i.annualProductionTopDown(true, 0.0).to(Exajoules))))

    val list2 = listValueVSCumulated(cells.map(i => (i.eroi(WattsPerSquareMeter(2), true, 7.0), i.annualProduction(WattsPerSquareMeter(2), true, 7.0).to(Exajoules))))
    val list10 = listValueVSCumulated(cells.map(i => (i.eroi(WattsPerSquareMeter(10), true, 7.0), i.annualProduction(WattsPerSquareMeter(10), true, 7.0).to(Exajoules))))
    val list = listValueVSCumulated(cells.map(i => (i.eroi(true, 7.0), i.annualProduction(true, 7.0).to(Exajoules))))

    PlotHelper.plotXY(List((list._1, list._2, "OptimalCD"), (list2._1, list2._2, "2 MW/km2"), (list10._1, list10._2, "10 MW/km2")), xLabel = "Potentiel Eolien [EJ/an]", yLabel = "EROI", legend = true)
    // printOptimalCDOnGrid(cells, 7.0, "res_7")
  }
  ...
  def plotOptimalCD(eroi_min: List[Double], cells: List[Cell]) {
    ...
    val fixedCD = List(2, 10, 20).map(i => WattsPerSquareMeter(i)).toList
    val f = fixedCD.map(cd => (eroiTest, eroiTest.map(e => energyProducedFixedCD(cells, cd, e, true).to(Exajoules)), cd.toString))
    PlotHelper.plotXY(List((eroi_min, optimalCD, "Optimal Capacity Density")) ++ f, xLabel = "Minimum EROI", yLabel = "Wind Potential [EJ/year]")
  }
  ...
  def plotEROIVSProd(eroi_min: List[Double], cells: List[Cell], topDown: Boolean = false) {
    val onshore = cells.filter(_.onshore)
    val offshore = cells.filter(i => !i.onshore)
    ...
    PlotHelper.plotXY(list, xLabel = "EROImin", yLabel = "Potential Global [EJ/an]")
  ...
  def energyProducedFixedCD(cells: List[Cell], cd: Irradiance, eroi_min: Double, suitable: Boolean = true) = {
    cells.map(c =>
      if (c.cf * GustavsonWakeEffect.wakeEffect(200, c.lambda(cd)) * 20 * 365 * 24 >= eroi_min * c.embodiedEnergy.to(MegawattHours))
        c.cf * c.area(suitable) * cd * GustavsonWakeEffect.wakeEffect(200, c.lambda(cd)) * Hours(365 * 24)
  ...
  def powerFixedCD(cells: List[Cell], cd: Irradiance, eroi_min: Double, suitable: Boolean = true) = {
    cells.map(c =>
      if (c.cf * GustavsonWakeEffect.wakeEffect(200, c.lambda(cd)) * 20 * 365 * 24 >= eroi_min * c.embodiedEnergy.to(MegawattHours))
        c.cf * c.area(suitable) * cd * GustavsonWakeEffect.wakeEffect(200, c.lambda(cd))
...
class Cell(val latitude: Double, val longitude: Double, val cf: Double, val totalArea: Area, val suitableArea: Area,
  ...
  def power(cd: Irradiance, suitable: Boolean, eroimin: Double): Power = {
    powerInstalled(cd, suitable, eroimin) * cf * GustavsonWakeEffect.wakeEffect(200, lambda(cd))
  }
