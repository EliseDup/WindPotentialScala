...
class Calibration2017(val T: Int = 15, val alpha: Double = 6 / 100.0, val m: Double = 5 / 100.0, val gpt: Double = 0.0 / 100, val theta: Double = 0.4, val energyUnits: EnergyUnit = KilowattHours) {
  def convertEnergy(energy: Energy) = energy.to(energyUnits)
  // Données observées
  // IEA
  val E = MegaTonOilEquivalent(9717.295)
  val Ce = MegaTonOilEquivalent(2064.274) + 0.5 * MegaTonOilEquivalent(2808.148)
  val Ef = E - Ce
  val Ee = MegaTonOilEquivalent(816.818)
  val Ye = E + Ee
  // Banque Mondiale
  val PIB_prev = 7.77968E+13 // PIB 2016
  val PIB = 8.02501E+13
  val g = (PIB - PIB_prev) / PIB_prev
  val s = 25.0 / 100
  // L=3422; % Pop active (10^6 personnes) % donnee non retenue
  val L = 2871 * 1E6; //* 1E6; // Pop employee (10^6 personnes) % donnee retenue
  ...
  val ve = Ke / convertEnergy(Ye)
  // pE = alpha PIB
  val p = alpha * PIB / convertEnergy(E)
  val Yf = PIB - p * convertEnergy(Ce)
  val qf = convertEnergy(Ef) / Yf; val vf = Kf / Yf;
  ...
  // autre variables
  val VAy = (1 - p * qf) * Yf;
  val VAe = p * convertEnergy(E);
  val I = s * PIB;
  val Cy = Yf - I;

  val n = p * convertEnergy(Ce) / Cy; // coefficient de partage du budget de C
  val sy = s * (1 + n) / (1 + s * n); // I/Y
  val e = convertEnergy(E) / PIB;
  val c = convertEnergy(Ce) / Cy;
  val k = K / convertEnergy(Ye);
