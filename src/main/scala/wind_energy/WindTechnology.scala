...
trait WindTechnology extends RenewableTechnology {
  ...
  override def embodiedEnergy(cell: Cell, eroi_min: Double): Energy = {
    (ratedPower(cell, eroi_min) / Gigawatts(1)) *
      ...
      operation(potential(cell, eroi_min) * Hours(365 * 24) * ee.lifeTime)
  }
  ...
  def power(cell: Cell, vr: Velocity, n: Double): Power = {
    ...
    if (top_down && res / (cell.suitabilityFactor(this) * cell.area) > cell.keDissipation) cell.keDissipation * cell.area * suitabilityFactor(cell)
    else res
    res
  }
